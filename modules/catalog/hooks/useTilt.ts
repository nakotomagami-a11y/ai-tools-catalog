...
export function useTilt<T extends HTMLElement>() {
  ...
  function handleEnter() {
    if (elRef.current) {
      elRef.current.style.transition = "transform 0.15s ease-out, border-color 0.3s";
    }
  }
  ...
  function handleLeave() {
    if (rafRef.current) cancelAnimationFrame(rafRef.current);
    if (elRef.current) {
      elRef.current.style.transition = "transform 0.5s cubic-bezier(.22,.68,0,1.2), border-color 0.3s";
      elRef.current.style.transform = "perspective(600px) rotateX(0deg) rotateY(0deg) scale3d(1,1,1)";
    }
  }
